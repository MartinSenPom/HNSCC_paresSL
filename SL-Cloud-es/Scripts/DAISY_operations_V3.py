...
def ProcessGeneAlias (client, input_gene_list, database):
    ...
    UNION DISTINCT
    SELECT DISTINCT Hugo_Symbol from `crucial-minutia-407312.DepMap_public_23Q4_backup.CCLE_mutation`   """
...
def GetTCGASubtypes(client):
    ...
    '''
    query="""
    SELECT DISTINCT TCGA_subtype from `crucial-minutia-407312.DepMap_public_23Q4_backup.sample_info`
    WHERE  primary_disease not in ('Non-Cancerous','Unknown','Engineered','Immortalized')
    AND EngineeredModel IS NULL
    AND Subtype NOT LIKE '%Immortalized%'
    """
...
def RetrieveSamples(client, data_resource, method, tissues):
    ...
    WHERE SampleType not like '%Normal%' and Study is not null '''
        ...
        sample_selection_sql= ''' SELECT  distinct ST.DepMap_ID FROM  `crucial-minutia-407312.DepMap_public_23Q4_backup.sample_info` ST,
        `isb-cgc-bq.DEPMAP.CCLE_gene_expression_DepMapPublic_current` E  
         WHERE ST.primary_disease 
        not in ('Non-Cancerous','Unknown','Engineered','Immortalized') AND ST.EngineeredModel IS NULL AND ST.Subtype NOT LIKE '%Immortalized%' AND E.DepMap_ID=ST.DepMap_ID '''
        if tissues.count('pancancer')==0:
            sample_selection_sql=sample_selection_sql + tissue_query
            sample_selection_sql=sample_selection_sql.replace('__TISSUE__', input_tissues)
        ...
        sample_selection_sql= '''SELECT distinct DepMap_ID, TCGA_subtype FROM
                ...
                FROM  `crucial-minutia-407312.DepMap_public_23Q4_backup.sample_info` ST,
   		 `isb-cgc-bq.DEPMAP.CCLE_gene_expression_DepMapPublic_current` E  WHERE  ST.primary_disease not
  		in ('Non-Cancerous','Unknown','Engineered','Immortalized') AND ST.EngineeredModel IS NULL AND ST.Subtype NOT LIKE '%Immortalized%' AND E.DepMap_ID=ST.DepMap_ID 
  		INTERSECT DISTINCT
   		SELECT  distinct ST.DepMap_ID  AS DepMap_ID, ST.TCGA_subtype as TCGA_subtype FROM `crucial-minutia-407312.DepMap_public_23Q4_backup.sample_info` ST,
   		`isb-cgc-bq.DEPMAP.CCLE_gene_cn_DepMapPublic_current`  C  WHERE  ST.primary_disease not
  		in ('Non-Cancerous','Unknown','Engineered','Immortalized') AND ST.EngineeredModel IS NULL AND ST.Subtype NOT LIKE '%Immortalized%' AND C.DepMap_ID=ST.DepMap_ID 
  		INTERSECT DISTINCT
  		SELECT  distinct ST.DepMap_ID  AS DepMap_ID, ST.TCGA_subtype as TCGA_subtype
  		FROM  `crucial-minutia-407312.DepMap_public_23Q4_backup.sample_info` ST,
  		`crucial-minutia-407312.DepMap_public_23Q4_backup.CCLE_mutation` M  WHERE  ST.primary_disease not
  		in ('Non-Cancerous','Unknown','Engineered','Immortalized') AND ST.EngineeredModel IS NULL AND ST.Subtype NOT LIKE '%Immortalized%' AND M.DepMap_ID=ST.DepMap_ID ) TS'''
        if tissues.count('pancancer')==0:
            sample_selection_sql=sample_selection_sql+ tissue_query
            sample_selection_sql=sample_selection_sql.replace('__TISSUE__', input_tissues)
        ...
        tissue_query=  " WHERE TS.TCGA_subtype in (__TISSUE__) "
        sample_selection_sql= '''SELECT distinct DepMap_ID, TCGA_subtype FROM
        (SELECT  distinct ST.DepMap_ID AS DepMap_ID , ST.TCGA_subtype AS TCGA_subtype FROM `crucial-minutia-407312.DepMap_public_23Q4_backup.sample_info` ST,     `isb-cgc-bq.DEPMAP.CCLE_gene_expression_DepMapPublic_current` E  WHERE  ST.primary_disease not
      in ('Non-Cancerous','Unknown','Engineered','Immortalized') AND ST.EngineeredModel IS NULL AND ST.Subtype NOT LIKE '%Immortalized%' AND E.DepMap_ID=ST.DepMap_ID 
      INTERSECT DISTINCT
      SELECT distinct ST.DepMap_ID AS DepMap_ID , ST.TCGA_subtype AS TCGA_subtype  FROM  `crucial-minutia-407312.DepMap_public_23Q4_backup.sample_info` ST,
      `isb-cgc-bq.DEPMAP.CCLE_gene_cn_DepMapPublic_current`  C  WHERE  ST.primary_disease not
      ...
      SELECT distinct ST.DepMap_ID AS DepMap_ID , ST.TCGA_subtype AS TCGA_subtype  FROM `crucial-minutia-407312.DepMap_public_23Q4_backup.sample_info` ST,
      `crucial-minutia-407312.DepMap_public_23Q4_backup.CCLE_mutation` M  WHERE ST.primary_disease not
      in ('Non-Cancerous','Unknown','Engineered','Immortalized') AND ST.EngineeredModel IS NULL AND ST.Subtype NOT LIKE '%Immortalized%' AND M.DepMap_ID=ST.DepMap_ID 
      INTERSECT DISTINCT
      SELECT distinct ST.DepMap_ID AS DepMap_ID , ST.TCGA_subtype AS TCGA_subtype  FROM  `crucial-minutia-407312.DepMap_public_23Q4_backup.sample_info` ST,
      `crucial-minutia-407312.DepMap_public_23Q4_backup.Achilles_gene_effect` A  WHERE  ST.primary_disease not
      in ('Non-Cancerous','Unknown','Engineered','Immortalized') AND ST.EngineeredModel IS NULL AND ST.Subtype NOT LIKE '%Immortalized%' AND A.DepMap_ID=ST.DepMap_ID)  TS'''
        if tissues.count('pancancer')==0:
            sample_selection_sql=sample_selection_sql+ tissue_query
            sample_selection_sql=sample_selection_sql.replace('__TISSUE__', input_tissues)
        selected_samples= list(client.query(sample_selection_sql).result().to_dataframe()['DepMap_ID'])
     ...
     `crucial-minutia-407312.DepMap_public_23Q4_backup.sample_info` ST,
      `isb-cgc-bq.DEPMAP.Combined_gene_dep_score_DEMETER2_current`   DS  WHERE ST.primary_disease not
        in ('Non-Cancerous','Unknown','Engineered','Immortalized') AND ST.EngineeredModel IS NULL AND ST.Subtype NOT LIKE '%Immortalized%' AND DS.CCLE_ID=ST.CCLE_Name
...
def SurvivalOfFittest(client, SL_or_SDL, data_source, input_genes, percentile_threshold, cn_threshold, adj_method, fdr_level, tissues, input_mutations='None'):
  ...
  if data_source=='PanCancerAtlas':
        ...
        mutation_table='crucial-minutia-407312.DepMap_public_23Q4_backup.CCLE_mutation'
...
def FunctionalExamination(client, SL_or_SDL, database, input_genes, percentile_threshold, cn_threshold, adj_method, fdr_level, tissues,  input_mutations=None):
   ...
   Output:
    ...
    if database=='CRISPR':
        ...
        dep_score_table='crucial-minutia-407312.DepMap_public_23Q4_backup.Achilles_gene_effect'
        sample_id='DepMap_ID'
        gene_exp='TPM'
        effect='Gene_Effect'
        symbol='Hugo_Symbol'
    ...
    gene_exp_table='isb-cgc-bq.DEPMAP.CCLE_gene_expression_DepMapPublic_current'
    cn_table='isb-cgc-bq.DEPMAP.CCLE_gene_cn_DepMapPublic_current'
    sample_info_table='crucial-minutia-407312.DepMap_public_23Q4_backup.sample_info'
    cn_threshold=np.log2(2**(cn_threshold)+1)
    gene_mapping=ProcessGeneAlias(client, input_genes, 'DepMap')
